...
# 
# This file is part of revealedPrefsmod.
# 
# revealedPrefsmod is free software: you can redistribute it and/or modify
# it under the terms of the GNU General Public License as published by
# the Free Software Foundation, either version 3 of the License, or
# (at your option) any later version.
# 
# revealedPrefsmod is distributed in the hope that it will be useful,
# but WITHOUT ANY WARRANTY; without even the implied warranty of
# MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
# GNU General Public License for more details.
# 
# You should have received a copy of the GNU General Public License
# along with revealedPrefsmod.  If not, see <http://www.gnu.org/licenses/>.
...
simGarp <- function(nobs, ngoods, afriat.par= 1, maxit= 10 * nobs, 
  ...
  res <- .Call("SimAxiom", nobs, ngoods, afriat.par, maxit, 
               pmin, pmax, qmin, qmax, "GARP", PACKAGE= "revealedPrefsmod")
...
simSarp <- function(nobs, ngoods, afriat.par= 1, maxit= 10 * nobs, 
  ...
  res <- .Call("SimAxiom", nobs, ngoods, afriat.par, maxit, 
               pmin, pmax, qmin, qmax, "SARP", PACKAGE= "revealedPrefsmod")
  if (res$nobs < nobs & res$iter == maxit)
    warning("Max iterations reached before completing requested size.")
  res
}
...
simWarp <- function(nobs, ngoods, afriat.par= 1, maxit= 10 * nobs, 
  ...
  res <- .Call("SimAxiom", nobs, ngoods, afriat.par, maxit, 
               pmin, pmax, qmin, qmax, "WARP", PACKAGE= "revealedPrefsmod")
  if (res$nobs < nobs & res$iter == maxit)
    warning("Max iterations reached before completing requested size.")
  res
}
...
################################################################################
## Iteratively generate random GARP-consistent observations
simGarpPrice <- function(nobs, ngoods, afriat.par= 1, maxit= 10 * nobs, 
                    qmin= 0, qmax= 1, pmin= 0, pmax= 1,p=matrix(rnorm(50*50),ncol=50,nrow=50)) {
  if (length(afriat.par) > 1 | afriat.par > 1 | afriat.par < 0)
    stop("'afriat.par' must be a real value between 0 and 1.\n")
  
  res <- .Call("SimAxiomPrice", nobs, ngoods, afriat.par, maxit, 
               pmin, pmax, qmin, qmax,"GARP", p, PACKAGE= "revealedPrefsmod")
  if (res$nobs < nobs & res$iter == maxit)
    warning("Max iterations reached before completing requested size.")
  res
}
...
##############################################################################
## New SimAxiomPriceWealth
################################################################################
## Iteratively generate random GARP-consistent observations
simGarpPriceWealth <- function(nobs, ngoods, afriat.par= 1, maxit= 10 * nobs, 
                         qmin= 0, qmax= 1, pmin= 0, pmax= 1,p=matrix(rnorm(50*50),ncol=50,nrow=50),w=matrix(rnorm(50*50),ncol=50,nrow=50)) {
  if (length(afriat.par) > 1 | afriat.par > 1 | afriat.par < 0)
    stop("'afriat.par' must be a real value between 0 and 1.\n")
  
  res <- .Call("SimAxiomPriceWealth", nobs, ngoods, afriat.par, maxit, 
               pmin, pmax, qmin, qmax,"GARP", p, w, PACKAGE= "revealedPrefsmod")
  if (res$nobs < nobs & res$iter == maxit)
    warning("Max iterations reached before completing requested size.")
  res
} 
...
################################################################################
## Iteratively generate random GARP-consistent observations
simGarpQuant <- function(nobs, ngoods, afriat.par= 1, maxit= 10 * nobs, 
                         qmin= 0, qmax= 1, pmin= 0, pmax= 1,q=matrix(rnorm(50*50),ncol=50,nrow=50)) {
  if (length(afriat.par) > 1 | afriat.par > 1 | afriat.par < 0)
    stop("'afriat.par' must be a real value between 0 and 1.\n")
  
  res <- .Call("SimAxiomQuant", nobs, ngoods, afriat.par, maxit, 
               pmin, pmax, qmin, qmax,"GARP", q, PACKAGE= "revealedPrefsmod")
  if (res$nobs < nobs & res$iter == maxit)
    warning("Max iterations reached before completing requested size.")
  res
}
...
##############################################################################
## New SimAxiomQuantWealth
################################################################################
## Iteratively generate random GARP-consistent observations
simGarpQuantWealth <- function(nobs, ngoods, afriat.par= 1, maxit= 10 * nobs, 
                               qmin= 0, qmax= 1, pmin= 0, pmax= 1,q=matrix(rnorm(50*50),ncol=50,nrow=50),w=matrix(rnorm(50*50),ncol=50,nrow=50)) {
  if (length(afriat.par) > 1 | afriat.par > 1 | afriat.par < 0)
    stop("'afriat.par' must be a real value between 0 and 1.\n")
  
  res <- .Call("SimAxiomQuantWealth", nobs, ngoods, afriat.par, maxit, 
               pmin, pmax, qmin, qmax,"GARP", q, w, PACKAGE= "revealedPrefsmod")
  if (res$nobs < nobs & res$iter == maxit)
    warning("Max iterations reached before completing requested size.")
  res
}