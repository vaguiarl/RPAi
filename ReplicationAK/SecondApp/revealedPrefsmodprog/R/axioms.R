...
checkWarp <- function(x, p, afriat.par= 1) {
  ...
  x <- as.matrix(x)
  p <- as.matrix(p)
  the.call <- .Call("CheckWarp", x, p, afriat.par, PACKAGE= "revealedPrefsmod")
  the.call$type <- "WARP"
...
checkSarp <- function(x, p, afriat.par= 1, method= c("deep", "floyd")) {
  ...
  if (method == "floyd") {
    res <- .Call("CheckSarp", x, p, afriat.par, PACKAGE= "revealedPrefsmod")
  } else {
    res <- .Call("DeepSarp", x, p, afriat.par, PACKAGE= "revealedPrefsmod")
    if (res$violation) {
      res$path <- res$path + 1
      cycle.start <- 
