...
RcppExport SEXP SimAxiomQuant(SEXP nobs, SEXP ngoods, SEXP afriat, SEXP maxit, 
                         SEXP pmin, SEXP pmax, SEXP qmin, SEXP qmax, 
                         SEXP axiom, SEXP q) { try {
  // import arguments
  ...
  while ((cur_length < n_obs) & (cur_it < max_it)) {
    ...
    cur_mat_p.insert_rows(cur_length, (p_max - p_min) * arma::randu(1, n_goods));
