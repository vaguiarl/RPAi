...
RcppExport SEXP SimAxiomQuantWealth(SEXP nobs, SEXP ngoods, SEXP afriat, SEXP maxit, 
                         SEXP pmin, SEXP pmax, SEXP qmin, SEXP qmax, 
                         SEXP axiom, SEXP q, SEXP w) { try {
  // import arguments
  ...
  arma::mat qmat(r_q.begin(), r_q.nrow(), r_q.ncol());
  ...
  mat_p=mat_p % wmat.row(0) / (dot(mat_p,qmat.row(0)) );
  //change
  //passing row_zero
  //arma::mat mat_p= pmat.row(0);
  //arma::mat mat_p= pmat.row(0);
  arma::mat mat_q= qmat.row(0);
  arma::mat cur_mat_q(mat_q);
  arma::mat cur_mat_p(mat_p);
  arma::mat test_mat_p= qmat.row(0);
  ...
  while ((cur_length < n_obs) & (cur_it < max_it)) {
    ...
    test_mat_p=(p_max - p_min) * arma::randu(1, n_goods);
    cur_mat_p.insert_rows(cur_length, test_mat_p % wmat.row(cur_length) / (dot(test_mat_p,qmat.row(cur_length)) ) );
    //cur_mat_p.insert_rows(0, (p_max - p_min) * arma::randu(1, n_goods));
	//change
	cur_mat_q.insert_rows(cur_length, qmat.row(cur_length));
