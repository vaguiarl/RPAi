...
RcppExport SEXP SimAxiomPriceWealth(SEXP nobs, SEXP ngoods, SEXP afriat, SEXP maxit, 
                         SEXP pmin, SEXP pmax, SEXP qmin, SEXP qmax, 
                         SEXP axiom, SEXP p, SEXP w) { try {
  // import arguments
  ...
  //change import price matrix
  NumericMatrix r_p(p);
  //unsigned n_obs= r_p.nrow();
  arma::mat pmat(r_p.begin(), r_p.nrow(), r_p.ncol());
  //change import w matrix
  NumericMatrix r_w(w);
  //unsigned n_obs= r_p.nrow();
  arma::mat wmat(r_w.begin(), r_w.nrow(), r_w.ncol());
  //change 
  //arma::mat mat_q= q_min + (q_max - q_min) * arma::randu(1, n_goods);
  //arma::mat mat_q= q_min + (q_max - q_min) * arma::randu(1, n_goods);
  //mat_q= mat_q/accu(mat_q.row(0)) % wmat.row(0) / pmat.row(0);
  arma::mat mat_q=  -log(arma::randu(1, n_goods));
  mat_q= mat_q/accu(mat_q.row(0)) % wmat.row(0) / pmat.row(0);
  ...
  arma::mat test_mat_q= pmat.row(0);
  ...
  while ((cur_length < n_obs) & (cur_it < max_it)) {
    ...
    //test_mat_q=(q_max - q_min) * arma::randu(1, n_goods);
    test_mat_q=-log(arma::randu(1, n_goods));
    //change
    cur_mat_q.insert_rows(cur_length,test_mat_q/accu(test_mat_q) % wmat.row(cur_length) / (pmat.row(cur_length)));
